...
int main()
{
  ...
  while (true)
  {
    ...
    else if (mode == Mode::SNAKE_32)
    {
      Snake::play_snake(fb, bit_streams);
      mode = Mode::DISPLAY;
    }
    else if (mode == Mode::SNAKE_8)
    {
      Snake::play_snake_8bit(fb, bit_streams);
      mode = Mode::DISPLAY;
    }
    else if (mode == Mode::SNAKE_4)
    {
      Snake::play_snake_4bit(fb, bit_streams);
      mode = Mode::DISPLAY;
    }
...
#define SNAKE_CODE_32bit   0x333      // DOWN DOWN DOWN
#define SNAKE_CODE_8bit    0x332      // DOWN DOWN LEFT
#define SNAKE_CODE_4bit    0x331      // DOWN DOWN UP
...
inline void handle_gamepad()
{
  ...
  if (game_pad.updated())
  {
    ...
    else if ((button_press_history & 0xFFF) == SNAKE_CODE_32bit)
    {
      mode = Mode::SNAKE_32;
      button_press_history = 0;
    }
    else if ((button_press_history & 0xFFF) == SNAKE_CODE_8bit)
    {
      mode = Mode::SNAKE_8;
      button_press_history = 0;
    }
    else if ((button_press_history & 0xFFF) == SNAKE_CODE_4bit)
    {
      mode = Mode::SNAKE_4;
      button_press_history = 0;
    }
